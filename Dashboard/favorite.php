      ...
      <div class="row row-cols-3 mt-4 mb-4">
        <?php
            // Define your table and column names (change as needed)
              ...
              $projects_favorite = "projects_favorite";
              $Project_Favorite_Status = "Project_Favorite_Status";
              $users = "users";
              $User_ID = "User_ID";
              $ID = $_SESSION['User_ID'];

              // Write the SQL query
              $sql = "SELECT $projects.*, $users.*,$projects_favorite.*,$project_categories.*
              FROM 
                $projects 
              INNER JOIN 
                $project_categories ON $project_categories.$Project_Categories_ID = $projects.$Project_Category 
              INNER JOIN 
                $projects_favorite ON $projects_favorite.$Project_ID = $projects.$Project_ID 
              INNER JOIN 
                $users ON $projects_favorite.$User_ID = $users.$User_ID 
              WHERE $projects_favorite.$User_ID LIKE $ID
              ORDER BY 
